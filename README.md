...
画布右上角的“Agent 接入”可复制当前画布连接说明给 Codex。支持创建节点、填写内容、连接素材、调用模型、取消运行和查询结果，命令复用现有画布操作。可以按当前画布保存各类节点的默认模型与参数，Agent 未指明时自动沿用，明确指定时只覆盖对应项。连接说明使用当前实际地址，支持启动器的本机局域网地址。Skill 由外部 Agent 管理，画布保留 LLM 执行、普通提示词和系统提示词。使用时保持本机画布页面打开。详见 [Agent 画布接口](static/agent-guide.md)，跨平台 CLI 为 `canvas_cli.py`。
